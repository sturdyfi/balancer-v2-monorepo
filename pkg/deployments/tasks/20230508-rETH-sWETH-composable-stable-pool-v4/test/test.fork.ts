...
import { BasePoolEncoder, StablePoolEncoder, SwapKind } from '@balancer-labs/balancer-js/src';
...
describeForkTest('ComposableStablePool V4', 'mainnet', 17217383, function () {
  let task: Task;

  let factory: Contract;
  let owner: SignerWithAddress;
  let whale: SignerWithAddress, swethWhale: SignerWithAddress, auraWhale: SignerWithAddress, graviAuraWhale: SignerWithAddress;
  let govMultisig: SignerWithAddress;
  let vault: Contract;
  let authorizer: Contract;
  let reth: Contract, sweth: Contract, aura: Contract, graviAura: Contract;

  const GOV_MULTISIG = '0x10A19e7eE7d7F8a52822f6817de8ea18204F2e4f';
  const RETH_HOLDER = '0x3ad1b118813e71a6b2683FCb2044122fe195AC36';
  const SWETH_HOLDER = '0x588711484FB02ba72316b59B944d93a691E74D60';
  const AURA_HOLDER = '0x2af2b2e485e1854fd71590c7ffd104db0f66f8a6';
  const GRAVIAURA_HOLDER = '0x36cc7B13029B5DEe4034745FB4F24034f3F2ffc6';

  const RETH = '0xae78736Cd615f374D3085123A210448E74Fc6393';
  const SWETH = '0xB1ebF8F77ae002d487e7Bde22c6180be0AF6a3d4';
  const AURA = '0xC0c293ce456fF0ED870ADd98a0828Dd4d2903DBF';
  const GRAVIAURA = '0xBA485b556399123261a5F9c95d413B4f93107407';

  const tokens = [RETH, SWETH];
  const amplificationParameter = bn(400);
  const swapFeePercentage = fp(0.01);
  const initialBalanceRETH = fp(40);
  const initialBalanceSWETH = fp(40);
  const initialBalances = [initialBalanceRETH, initialBalanceSWETH];
  ...
  before('load signers', async () => {
    ...
    whale = await impersonate(RETH_HOLDER, fp(100));
    swethWhale = await impersonate(SWETH_HOLDER, fp(100));
    auraWhale = await impersonate(AURA_HOLDER, fp(100));
    graviAuraWhale = await impersonate(GRAVIAURA_HOLDER, fp(100));
  ...
  before('load vault and tokens', async () => {
    ...
    reth = await task.instanceAt('ERC20', RETH);
    sweth = await task.instanceAt('ERC20', SWETH);
    aura = await task.instanceAt('ERC20', AURA);
    graviAura = await task.instanceAt('ERC20', GRAVIAURA);

    await sweth.connect(swethWhale).transfer(whale.address, initialBalanceSWETH.mul(10));

    await reth.connect(whale).approve(vault.address, MAX_UINT256);
    await sweth.connect(whale).approve(vault.address, MAX_UINT256);
  });
  ...
  describe('pool operations', () => {
    ...
    context('swap', () => {
      ...
      it('performs a swap', async () => {
        await reth.connect(whale).transfer(owner.address, amount);
        await reth.connect(owner).approve(vault.address, amount);

        await vault
          .connect(owner)
          .swap(
            { kind: SwapKind.GivenIn, poolId, assetIn: RETH, assetOut: SWETH, amount, userData: '0x' },
        ...
        // Assert pool swap
        const expectedSWETH = amount;
        expectEqualWithError(await reth.balanceOf(owner.address), 0, 0.0000001);
        expectEqualWithError(await sweth.balanceOf(owner.address), bn(expectedSWETH), 0.03);
      });
    });
    ...
    context('proportional join', () => {
      ...
      it('joins proportionally', async () => {
        ...
        const adjustedBalances = [
          initialBalanceRETH.div(fp(4.99)).mul(fp(1)),
          initialBalanceSWETH.div(fp(4.99)).mul(fp(1)),
        ];
  ...
  describe('read-only reentrancy protection', () => {
    ...
    sharedBeforeEach('deploy and fund attacker', async () => {
      ...
      await reth.connect(whale).transfer(attacker.address, attackerFunds);
      await sweth.connect(whale).transfer(attacker.address, attackerFunds);
  ...
  describe('recovery mode', () => {
    ...
    it('can exit via recovery mode', async () => {
      ...
      const vaultSWETHBalanceBeforeExit = await sweth.balanceOf(vault.address);
      const ownerSWETHBalanceBeforeExit = await sweth.balanceOf(owner.address);
      ...
      const vaultSWETHBalanceAfterExit = await sweth.balanceOf(vault.address);
      const ownerSWETHBalanceAfterExit = await sweth.balanceOf(owner.address);

      expect(vaultSWETHBalanceAfterExit).to.lt(vaultSWETHBalanceBeforeExit);
      expect(ownerSWETHBalanceAfterExit).to.gt(ownerSWETHBalanceBeforeExit);
    });
  });
